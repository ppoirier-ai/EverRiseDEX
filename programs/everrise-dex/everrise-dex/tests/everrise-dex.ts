...
describe("everrise-dex", () => {
  ...
  it("Initialize EverRise DEX", async () => {
    ...
    expect(bondingCurve.x.toNumber()).to.equal(100_000_000_000); // 100,000 USDC
    expect(bondingCurve.y.toNumber()).to.equal(1_000_000_000_000_000); // 1B EVER
    expect(bondingCurve.k.toNumber()).to.equal(100_000_000_000_000_000_000_000); // K = X * Y
    expect(bondingCurve.queueHead.toNumber()).to.equal(0);
    expect(bondingCurve.queueTail.toNumber()).to.equal(0);
  });
  ...
  it("Calculate initial price", async () => {
    ...
    const expectedPrice = 100_000_000_000 / 1_000_000_000_000_000; // 0.0001 in proper decimals
  ...
  it("Test bonding curve math", async () => {
    ...
    const y = 1_000_000_000_000_000; // 1,000,000,000 EVER
    const k = x * y;
